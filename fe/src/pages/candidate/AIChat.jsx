...
const STORAGE_KEY = 'candidate_ai_chat_sessions';
...
const AIChat = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await aiChatService.candidateChat(payload);
      const messageText = response?.message || 'No reply from AI.';
      const state = response?.data?.state || {};
      const result = response?.data?.result || {};
      const suggestions = result.suggestions || result.jobs || [];
      const jobs = buildJobsList(suggestions);
      ...
      updateSession(currentSessionId, (s) => ({
        ...
        jobId: state.payload?.selected_job_id || state.payload?.job_id || s.jobId,
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="mt-2 text-gray-600">
          Trợ lý tìm việc giúp gợi ý job, chấm fit và lộ trình học.
        </p>
      </div>
            ...
            <div className="px-6 py-4 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Trợ lý tìm việc</p>
                </div>
                {activeSession?.jobId ? (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              {(activeSession?.messages || []).length === 0 ? (
                <div className="text-sm text-gray-500">
                  Gợi ý: "xem CV tôi tìm job phù hợp" hoặc "tìm job backend ở Đà Nẵng remote".
                </div>
              ) : (
            ...
            <div className="border-t border-gray-100 px-6 py-4">
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  className="flex-1 resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
                <button
                  onClick={handleSend}
                  disabled={isSending}
