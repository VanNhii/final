...
const RecruiterProfile = () => {
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Update Redux state immediately for header preview
        dispatch(updateUserData({avatar_url: response.data.file_url}));
  ...
  const handleCoverUpload = async (e) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        setCompanyInfo(prev => ({
          ...prev,
          cover_image_url: response.data.file_url
        }));
  ...
  const loadProfile = async () => {
    try {
      ...
      setCompanyInfo({
        ...
        company_locations: profileData.company_locations || []
      });
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      // Update User model to ensure avatar and name persist on refresh
      if (profile?.user_id?._id) {
        const nameParts = cleanedProfileData.contact_person_name?.trim().split(' ') || [];
        const userUpdateData = {
          avatar_url: cleanedProfileData.avatar_url || cleanedProfileData.logo_url,
          first_name: nameParts[0] || user?.first_name || '',
          last_name: nameParts.length > 1 ? nameParts.slice(1).join(' ') : (user?.last_name || '')
        };
        await recruiterService.updateUserProfile(profile.user_id._id, userUpdateData);
      }

      // Update Redux state to reflect changes in header
      dispatch(updateUserData({
        full_name: cleanedProfileData.contact_person_name,
        // Use personal avatar if available, otherwise fallback to company logo
        avatar_url: cleanedProfileData.avatar_url || cleanedProfileData.logo_url
      }));
  ...
  const addLocation = () => {
    setCompanyInfo(prev => ({
      ...prev,
      company_locations: [...prev.company_locations, {city: '', address: '', is_headquarters: false}]
    }));
  };
  ...
  const removeLocation = (index) => {
    setCompanyInfo(prev => ({
      ...prev,
      company_locations: prev.company_locations.filter((_, i) => i !== index)
    }));
  };

  const setHeadquarters = (index) => {
    setCompanyInfo(prev => ({
      ...prev,
      company_locations: prev.company_locations.map((location, i) => ({
        ...location,
        is_headquarters: i === index
      }))
    }));
  };
  ...
  const tabs = [
    ...
    {id: 'locations', label: 'Chi nhánh', icon: FaMapMarkerAlt},
        ...
        <div className="bg-white rounded-lg shadow-sm mb-6 overflow-hidden">
          ...
          <div className="h-48 bg-gradient-to-r from-green-500 to-blue-600 relative">
            {companyInfo.cover_image_url && (
              <img
                src={getAvatarUrl(companyInfo.cover_image_url)}
                alt="Cover"
            ...
            {editMode && (
              <>
                ...
                <button
                  ...
                  className="absolute top-4 right-4 bg-white bg-opacity-80 rounded-full p-2 hover:bg-opacity-100 transition-all shadow-md active:scale-95"
                >
          ...
          <div className="px-6 py-4">
            <div className="flex items-center space-x-4">
              <div className="relative -mt-16">
                <div className="relative w-24 h-24 bg-white rounded-full p-1 shadow-lg">
                  {companyInfo.logo_url ? (
                    <img
                      src={getAvatarUrl(companyInfo.logo_url)}
                      alt="Logo"
                  ...
                  {editMode && (
                    <>
                      ...
                      <button
                        ...
                        className="absolute bottom-0 right-0 bg-green-600 rounded-full p-1.5 text-white hover:bg-green-700 transition-colors shadow-md active:scale-95 z-10"
                      >
...
const LocationsTab = ({
  ...
  <div className="space-y-6">
    <div className="flex items-center justify-between mb-4">
      <h3 className="text-lg font-semibold text-gray-900">Các chi nhánh / Văn phòng</h3>
      {editMode && (
        <button
            ...
            {editMode ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex items-center space-x-4">
                    {!location.is_headquarters && (
                      <button
                        onClick={() => onSetHeadquarters(index)}
              ...
              <div className="space-y-2">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900">{location.city || 'Chưa có thành phố'}</p>
                    <p className="text-gray-600 text-sm">{location.address || 'Chưa có địa chỉ'}</p>
                  </div>
                </div>
              </div>
            )}
...
const PersonalInfoTab = ({
  ...
  <div className="space-y-6">
    ...
    <div className="flex flex-col items-center mb-6">
      <div className="relative group">
        <div className="w-32 h-32 bg-gray-100 rounded-full flex items-center justify-center overflow-hidden border-4 border-white shadow-md">
          {personalInfo.avatar_url ? (
            <img src={getAvatarUrl(personalInfo.avatar_url)} alt="Avatar" className="w-full h-full object-cover" />
          ) : (
        ...
        {editMode && (
          <>
            ...
            <button
              ...
              className="absolute bottom-0 right-0 bg-green-600 p-2 rounded-full text-white shadow-lg hover:bg-green-700 transition-all active:scale-95"
            >
      ...
      <p className="mt-2 text-sm text-gray-500 font-medium">Ảnh đại diện cá nhân</p>
    </div>
