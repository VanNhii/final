...
class RecommendationEngine:
    ...
    def recommend_jobs_for_candidate(self, candidate_id, limit=20, filters=None):
        ...
        if self.config.CACHE_ENABLED and self.config.RECOMMEND_CACHE_TTL > 0:
            has_filters = bool(filters) and any(v not in [None, "", []] for v in filters.values())
            if not has_filters:
                cached = self._get_cached_job_recommendations(candidate_id, limit)
                if cached is not None:
                    logger.info(f"Returning cached recommendations for candidate {candidate_id}")
                    return cached
        ...
        job_query = self._build_job_query(filters)
        ...
        for job in jobs:
            try:
                ...
                if score_data['score'] >= self.config.MIN_CONFIDENCE_SCORE:
                    recommendations.append(scored_jobs[-1])
            except Exception as e:
        ...
        # If nothing meets threshold, return top scored jobs
        if not recommendations and scored_jobs:
            logger.info("No recommendations met threshold; returning top scored jobs")
    ...
    def _get_cached_job_recommendations(self, candidate_id, limit):
        try:
            cutoff = datetime.utcnow() - timedelta(seconds=self.config.RECOMMEND_CACHE_TTL)
            recs = self.db.get_collection('airecommendations').find(
                {
                    'requester_id': ObjectId(candidate_id),
                    'requester_type': 'candidate',
                    'recommendation_type': 'job_for_candidate',
                    'created_at': {'$gte': cutoff}
                }
            ).sort([('created_at', -1), ('score', -1)]).limit(limit)
            recs = list(recs)
        except Exception as exc:
            logger.warning(f"Failed to load cached recommendations: {exc}")
            return None

        if not recs:
            return None

        job_ids = [rec.get('recommended_entity_id') for rec in recs if rec.get('recommended_entity_id')]
        if not job_ids:
            return None

        jobs = self.db.get_collection('jobs').find({'_id': {'$in': job_ids}})
        job_map = {str(job['_id']): job for job in jobs}

        out = []
        for rec in recs:
            job = job_map.get(str(rec.get('recommended_entity_id')))
            if not job:
                continue
            out.append({
                'job': job,
    ...
    def _score_candidate_job_match(self, candidate, job):
        ...
        if self.content_model.model is not None:
            try:
                ...
                if not df.empty and len(feature_columns) > 0:
                    ...
                    proba = self.content_model.predict_proba(X)
        ...
        if self.cf_model.user_factors is not None:
            try:
    ...
    def _has_sufficient_profile(self, candidate):
        ...
        Returns:
            Boolean indicating if profile has enough data
        """
        # Check for skills
        ...
        skills_detailed = candidate.get('skills_detailed', [])
        if skills_detailed and len(skills_detailed) > 0:
            return True
    ...
    def _get_popular_jobs(self, limit=20, filters=None):
        ...
        Returns:
            List of recommended jobs with default scores
        """
        ...
        # Format as recommendations with default score
        recommendations = []
        for job in jobs:
            recommendations.append({
                'job': job,
                'score': 0.5,  # Default score for popular jobs
                'confidence': 0.3,  # Low confidence since not personalized
                'reasons': ['Công việc mới đăng tuyển', 'Phổ biến trong hệ thống']
            })
    ...
    def _build_job_query(self, filters=None):
        clauses = [
        ...
        if filters:
            if filters.get('location'):
                clauses.append({'location.city': filters['location']})
            if filters.get('job_type'):
                clauses.append({'job_type': filters['job_type']})
            if filters.get('salary_min'):
                clauses.append({'salary_min': {'$gte': filters['salary_min']}})
