...
exports.getDashboardStats = async (req, res, next) => {
  try {
    ...
    // Get recent pending jobs for quick approval
    ...
    // Get stats for growth calculations (Rolling 30 days)
    const now = new Date();
    const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    const sixtyDaysAgo = new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000);
    
    const [
      newUsers30d, prevUsers30d, 
      newJobs30d, prevJobs30d, 
      newApps30d, prevApps30d,
      currMonthRevArr, prevMonthRevArr
    ] = await Promise.all([
      User.countDocuments({ created_at: { $gte: thirtyDaysAgo } }),
      User.countDocuments({ created_at: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } }),
      Job.countDocuments({ created_at: { $gte: thirtyDaysAgo } }),
      Job.countDocuments({ created_at: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } }),
      Application.countDocuments({ created_at: { $gte: thirtyDaysAgo } }),
      Application.countDocuments({ created_at: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } }),
      Payment.aggregate([
        { $match: { payment_status: 'completed', created_at: { $gte: thirtyDaysAgo } } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
      ]),
      Payment.aggregate([
        { $match: { payment_status: 'completed', created_at: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
      ])
    ]);

    const currMonthRev = currMonthRevArr[0]?.total || 0;
    const prevMonthRev = prevMonthRevArr[0]?.total || 0;
    ...
    const userGrowthPercent = calculateGrowth(newUsers30d, prevUsers30d);
    const jobGrowthPercent = calculateGrowth(newJobs30d, prevJobs30d);
    const applicationGrowthPercent = calculateGrowth(newApps30d, prevApps30d);
    const revenueGrowthPercent = calculateGrowth(currMonthRev, prevMonthRev);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        totalRevenue: Math.round(totalRevenue / 1000000),
...
exports.getJobs = async (req, res, next) => {
  try {
    ...
    // Create base query WITHOUT status for stats
    const baseQuery = { ...searchFilters };
    if (category_id) {
      baseQuery.categories = category_id;
    }
    // Remove status from baseQuery if it was added by getSearchParams
    ...
    const [jobs, totalJobs, stats] = await Promise.all([
      ...
      Promise.all([
        ...
        Job.countDocuments({ ...baseQuery, status: 'pending' }),
        Job.countDocuments({ ...baseQuery, status: 'approved' }),
        Job.countDocuments({ ...baseQuery, status: 'rejected' })
      ])
    ]);
